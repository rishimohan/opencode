...
import { canonical, DirectoryUnavailableError } from "./worktree/directory"
import { WorktreeGit } from "./worktree/git"
import type { EffectDrizzleSqlite } from "@opencode-ai/effect-drizzle-sqlite"

export { DirectoryUnavailableError } from "./worktree/directory"
...
export const ListOutput = Worktree.ListOutput
...
export interface Interface {
  ...
  readonly list: (projectID: ProjectSchema.ID) => Effect.Effect<ListOutput>
...
export const node = makeGlobalNode({
  service: Service,
  layer: layer,
  deps: [FSUtil.node, Git.node, EventV2.node, Database.node],
})
