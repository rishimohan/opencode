...
const it = testEffect(AppNodeBuilder.build(LayerNode.group([Worktree.node, Database.node, EventV2.node])))
...
describe("Worktree", () => {
  ...
  it.effect("rejects duplicate strategies and reports unavailable ids", () =>
    Effect.gen(function* () {
      ...
      const strategy: Worktree.Strategy = {
        id: Worktree.StrategyID.make("test/duplicate"),
        create: () => Effect.die("unused"),
        remove: () => Effect.die("unused"),
        list: () => Effect.succeed([]),
      }

      yield* worktree.register(strategy)
      expect(yield* worktree.register(strategy).pipe(Effect.flip)).toBeInstanceOf(Worktree.DuplicateStrategyError)
  ...
  it.live("requires a tracked project root", () =>
    Effect.gen(function* () {
      const input = yield* setup()
      yield* input.db
      ...
      const error = yield* worktree
        .create({
          ...
          directory: abs(`${input.root.path}-missing-root`),
          name: "worktree",
        })
        .pipe(Effect.flip)
  ...
  it.live("does not publish an event when refresh finds no directory changes", () =>
    Effect.gen(function* () {
      ...
      const worktree = yield* Worktree.Service
      const events = yield* EventV2.Service
      const event = yield* events.subscribe(Worktree.Event.Updated).pipe(
        Stream.take(1),
        Stream.runCollect,
        Effect.forkScoped,
